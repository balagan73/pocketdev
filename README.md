...
- **`extensions/`** — Available extensions: `telegram/`, `piper/`, `whisper/`. Each has `manifest.yaml`, `install.sh`, and optional `config.yaml`.
- **`.openclaw/`** — Container and orchestration:
  - `Dockerfile` — copies pocket-dev files into the image, installs ffmpeg (needed by whisper), runs `orchestrate.sh`.
